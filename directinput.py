...
if IS_WINDOWS:
    ...
    def TypeString(text, interval=0.25):
        from pyautogui import typewrite
        typewrite(text, interval=interval)

else:
    # Linux: inject through a uinput virtual keyboard (see linux_input.py).
    import linux_input

    def PressKey(hexKeyCode):
        linux_input.keyboard().key(hexKeyCode, True)

    def ReleaseKey(hexKeyCode):
        linux_input.keyboard().key(hexKeyCode, False)

    def TypeString(text, interval=0.25):
        linux_input.keyboard().type_string(text, interval=interval)
